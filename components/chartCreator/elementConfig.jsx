                    ...
                    {!el.manualEntry && (
                        <div>
                            ...
                            {chartType === "Map" && (
                                <>
                                    <div>
                                        <Label htmlFor={`latitudeKey-${el.id}`}>Latitude Key</Label>
                                        <Select
                                            value={el.latitudeKey}
                                            onValueChange={(value) => handleElementChange(el.id, "latitudeKey", value)}
                                            className="dark:bg-gray-700"
                                        >
                                            <SelectTrigger id={`latitudeKey-${el.id}`} className="dark:bg-gray-700">
                                                <SelectValue placeholder="Select Latitude Key"/>
                                            </SelectTrigger>
                                            <SelectContent>
                                                {availableKeys.map((key) => (
                                    ...
                                    <div>
                                        <Label htmlFor={`longitudeKey-${el.id}`}>Longitude Key</Label>
                                        <Select
                                            value={el.longitudeKey}
                                            onValueChange={(value) => handleElementChange(el.id, "longitudeKey", value)}
                                            className="dark:bg-gray-700"
                                        >
                                            <SelectTrigger id={`longitudeKey-${el.id}`} className="dark:bg-gray-700">
                                                <SelectValue placeholder="Select Longitude Key"/>
                                            </SelectTrigger>
                                            <SelectContent>
                                                {availableKeys.map((key) => (
                            ...
                            {chartType === "Map Trajectory" && (
                                <>
                                    ...
                                    <div>
                                        <Label htmlFor={`timestampKey-${el.id}`}>Timestamp Key</Label>
                                        <Select
                                            value={el.timestampKey}
                                            onValueChange={(value) => handleElementChange(el.id, "timestampKey", value)}
                                            className="dark:bg-gray-700"
                                        >
                                            <SelectTrigger id={`timestampKey-${el.id}`} className="dark:bg-gray-700">
                                                <SelectValue placeholder="Select Timestamp Key"/>
                                            </SelectTrigger>
                                            <SelectContent>
                                                {availableKeys.map((key) => (
                                                    <SelectItem key={key} value={key}>
