// TODO: meer als options.interval seconde tussen de punten bij traject playback en er is een verlies van data waarvoor de traject niet meer klopt
...
export const renderOther = ({ chartType, elements, graphData }) => {
    ...
    switch (chartType) {
        ...
        case "Map":
            ...
            return (
                <MapContainer center={[0, 0]} zoom={2} style={{ height: "80%", width: "100%" }}>
...
const TrajectoryPlayback = ({ elements, graphData }) => {
  ...
  const elapsedTime =
    trajectoryPoints.length > 1
      ? trajectoryPoints[playbackIndex].timestamp - trajectoryPoints[0].timestamp
      : 0;
  ...
  const remainingTime =
    trajectoryPoints.length > 1
      ? Math.max(
          0,
          trajectoryPoints[trajectoryPoints.length - 1].timestamp - trajectoryPoints[playbackIndex].timestamp
        )
      : 0;

  const getStatusText = () => {
    if (!isPlaying && playbackIndex === 0) return "Ready";
    if (!isPlaying && playbackIndex >= trajectoryPoints.length - 1) return "Finished";
    return isPlaying ? `Playing ${speed}x` : "Paused";
  };
      ...
      <div className="bg-gray-800 p-4">
        ...
        <div className="flex items-center justify-between mt-4">
          <div className="flex items-center space-x-2">
            <Button variant="ghost" size="icon" onClick={() => setPlaybackIndex(Math.max(playbackIndex - 1, 0))}>
            ...
            <Button variant="ghost" size="icon" onClick={() => setPlaybackIndex(Math.min(playbackIndex + 1, trajectoryPoints.length - 1))}>
