...
const TrajectoryPlayback = ({ elements, graphData }) => {
    ...
    useEffect(() => {
        ...
        if (isPlaying && playbackIndex < trajectoryPoints.length - 1) {
            const timeDiff =
                trajectoryPoints[playbackIndex + 1]?.timestamp - trajectoryPoints[playbackIndex]?.timestamp;
            interval = setTimeout(() => {
                setPlaybackIndex((prevIndex) => Math.min(prevIndex + 1, trajectoryPoints.length - 1));
            }, timeDiff / speed);
        }
        return () => clearTimeout(interval);
    }, [isPlaying, playbackIndex, trajectoryPoints, speed]);
    ...
    return (
        <div>
            ...
            <div className="flex items-center space-x-4 mt-2">
                ...
                <p>
                    {trajectoryPoints.length > 0 &&
                        new Date(trajectoryPoints[playbackIndex].timestamp).toLocaleTimeString()}
                </p>
            </div>
        </div>
    );
