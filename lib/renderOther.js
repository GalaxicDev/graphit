        ...
        <div className="flex items-center justify-between mt-4">
          <div className="flex items-center space-x-2">
            <Button variant="ghost" size="icon" onClick={handleFastBackward}>
              <SkipBack className="w-5 h-5 text-gray-300" color="#fff"/>
            </Button>
            <Button variant="ghost" size="icon" onClick={() => setIsPlaying(!isPlaying)}>
              {isPlaying ? <Pause className="w-5 h-5" color="#fff" /> : <Play className="w-5 h-5" color="#fff"/>}
            </Button>
            <Button variant="ghost" size="icon" onClick={handleFastForward}>
              <SkipForward className="w-5 h-5" color="#fff" />
            </Button>
            <div className="text-gray-300 text-sm font-bold">
              <p className="text-gray-300"> {new Date(elapsedTime).toISOString().substr(11, 8)} / {new Date(remainingTime).toISOString().substr(11, 8)}</p>
            </div>
          </div>
          <div className="items-center space-x-2 hidden"> {/* Hidden for now, if unhidden add flex class */}
            <DropdownMenu>
